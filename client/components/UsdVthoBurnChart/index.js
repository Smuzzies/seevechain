...
export default function UsdVthoBurnChart({ forcePageUpdate }) {
  ...
  if (
    ...
    return (
      <div className="TopContractsCharts">
        <Spinner />
      </div>
    );

  const { groups, filteredContracts } = getGroupsAndFilteredContractsFromTopContracts(
    topContracts
  );

  const contractsWithUsdBurned = filteredContracts.map((contract) => ({
    ...contract,
    usdBurned: contract.vthoBurn * vthoPrice,
  }));

  const usdVthoBurnTopContracts = [...contractsWithUsdBurned]
    .sort((a, b) => b.usdBurned - a.usdBurned)
    .slice(0, 20);
    ...
    <div className="TopContractsCharts">
      ...
      <div
        ...
        onClick={onContractClick({
          contracts: usdVthoBurnTopContracts,
          groups,
          forcePageUpdate,
        })}
      >
      ...
      <LineChart
        ...
        modifyOptions={(options) => {
          options.scales.yAxes[0].ticks.userCallback = (num) =>
